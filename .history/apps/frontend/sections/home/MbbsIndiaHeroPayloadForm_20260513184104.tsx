...
export function MbbsIndiaHeroPayloadForm({
  ...
  const [form, setForm] = useState<PayloadFormDoc | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const res = await fetch('/api/cms/forms/mbbs-india', { cache: 'no-store' });
        const data = (await res.json()) as FormsListResponse & { message?: string };
        if (!res.ok) {
          throw new Error(data.message || `Could not load form (${res.status})`);
        }
        const doc = data.docs?.[0];
        if (!doc || !doc.id) {
          throw new Error('No form named "MBBS INDIA" found in Payload. Check the form title in CMS.');
        }
        if (cancelled) return;
        setForm(doc);
