...
export const MBBSIndiaStateSection = () => {
  ...
  const collegeCountLabel = currentState
    ? `${currentState.colleges.length} Medical colleges`
    : ''

  return (
    <section
      className="relative overflow-hidden bg-slate-50 py-16 md:py-24"
      id="mbbs-india-colleges"
      data-college-label="medical-v2"
    >      <div
        ...
        {currentState ? (
          <div key={currentState.id}>
            <div className="mb-8 flex flex-wrap items-end justify-between gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm md:p-8">
              <div>
                <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-gold-600">
                  <MapPin className="h-3.5 w-3.5" aria-hidden />
                  <span>{currentState.name}</span>
                </div>
                <h3 className="mt-2 text-2xl font-bold text-navy-900 md:text-3xl">{collegeCountLabel}</h3>
              </div>
              <Link href={currentState.href} className="site-gold-cta inline-flex items-center gap-2">
