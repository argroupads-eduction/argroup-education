/**
 * Copy Payload cms schema data: Supabase â†’ CockroachDB.
...
function clientConfig(url) {
  ...
  if (isCockroach(url)) {
    ...
    // Supabase / Neon pooler: Windows often hits SELF_SIGNED_CERT_IN_CHAIN
...
async function main() {
  ...
  if (!isCockroach(dstInfo.url)) {
    console.error('Destination is not Cockroach â€” aborting for safety')
    process.exit(1)
  }
  if (isCockroach(srcInfo.url)) {
    console.error('Source looks like Cockroach â€” aborting for safety')
    process.exit(1)
  }
